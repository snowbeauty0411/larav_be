...
class NumberAccessListServicePage extends Model
{
    use HasFactory;
    protected $table = 'number_access_list_service_pages';
    ...
    public function countNumberAccessServiceList($services)
    {
        if (auth(UserConst::USER_GUARD)->user()) {
            foreach ($services as $service) {
                $number_access_list_service = $this->findLastByService($service->id);
                if (auth(UserConst::USER_GUARD)->user()->id != $service->seller_id) {
                    if (!$number_access_list_service) {
                        ...
                        if ($created_date->isCurrentDay()) {
                            $number_access_list_service->update([
                                'count_by_month' => $number_access_list_service->count_by_month + 1
                            ]);
                        } else {
                            $this->create([
                                'service_id' => $service->id,
    ...
    public function redisCountNumberAccessServiceList($services)
    {
        ...
        if (auth(UserConst::USER_GUARD)->user()) {
            foreach ($services as $service) {
                if (auth(UserConst::USER_GUARD)->user()->id != $service->seller_id) {
                    $redisServices[$service->id] = isset($redisServices[$service->id]) ?  $redisServices[$service->id] + 1 : 1;
                }
            }
        } else {
            foreach ($services as $service) {
        ...
        $redis->set('redis_service_access', json_encode($redisServices));
    }
    ...
    public function statisticalByServiceMonth($service_id)
    {
        ...
        $result_current_month = $this->where('service_id', $service_id)
            ->whereMonth('created_at', $curr_month)
            ->whereYear('created_at', '=', Carbon::now()->year)
            ->select('count_by_month')->first();
        $result_last_month = $this->where('service_id', $service_id)
            ->whereMonth('created_at', $curr_month - 1)
            ->whereYear('created_at', '=', Carbon::now()->year)
            ->select('count_by_month')->first();

        $current_access_list_pages = 0;

        $last_month_access_list_pages = 0;
        if ($result_current_month) {
            $current_access_list_pages = $result_current_month->count_by_month;
        }

        if ($result_last_month) {
            $last_month_access_list_pages = $result_last_month->count_by_month;
        }

        $increase_number = $current_access_list_pages - $last_month_access_list_pages;
        $result['current_access_list_pages'] = $current_access_list_pages;
    ...
    public function statisticalByServiceYear($service_id, $condition)
    {
        ...
        $result['current_access_list_pages'] = $result_current_year;
        $result['increase_number'] = $increase_number;
