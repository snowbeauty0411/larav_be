...
class NumberClickOfficialUrl extends Model
{
    use HasFactory;

    protected $table = 'number_click_official_url';
    ...
    public function statisticalByServiceMonth($service_id)
    {
        ...
        $current_number_click_official_urls = 0;
        $last_month_number_click_official_urls = 0;
        if ($result_current_month) {
            $current_number_click_official_urls = $result_current_month->count_by_month;
        }

        if ($result_last_month) {
            $last_month_number_click_official_urls = $result_last_month->count_by_month;
        }
        $increase_number = $current_number_click_official_urls - $last_month_number_click_official_urls;
        $result['current_number_click_official_urls'] = $current_number_click_official_urls;
    ...
    public function graphByService($service_id, $condition)
    {
        ...
        if ($month == 1) {
            ...
            $graph =  $this->where('service_id', $service_id)->select(
                ...
                DB::raw('SUM(count_by_month) as count_click'),
            )
            ...
            foreach ($results as $key => $item) {
                foreach ($graph as $graph_item) {
                    if ($item['year'] == $graph_item['year'] && $item['month'] == $graph_item['month'] && $item['day'] == $graph_item['day']) {
                        $item['value'] = $graph_item['count_click'];
                        $results[$key] = $item;
                    }
                }
            }
    ...
    public function statisticalByServiceYear($service_id, $condition)
    {
        ...
        $result['current_number_click_official_urls'] = $result_current_year;
        $result['increase_number'] = $increase_number;

        return $result;
    }

    public function countNumberClickOfficialUrl($number_click,$service_id){
        if (!$number_click) {
            $this->create([
            ...
            $created_date = Carbon::parse($number_click->created_at);
            if ($created_date->isCurrentDay()) {
                $number_click->update([
                    'count_by_month' => $number_click->count_by_month + 1
                ]);
            } else {
                $this->create([
                    'service_id' => $service_id,
